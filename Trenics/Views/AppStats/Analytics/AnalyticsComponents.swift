...
/// Section chrome for the analytics detail pages: a title, an optional
/// subtitle, the content, and a rule underneath. Deliberately has no card
/// background — on a page that is almost entirely sections, stacked cards read
/// as clutter, and a divider separates them just as clearly.
...
/// "What does it mean?" — a collapsed row that explains the numbers above it in
/// plain language.
///
/// The explanation is generated on expand rather than up front. A page carries
/// half a dozen of these, and generating them all eagerly would mean half a
/// dozen model round trips for text the user may never open. Once generated it
/// is kept until the underlying figures change.
///
/// `fallback` is a written explanation of what the metric means, shown when
/// on-device generation isn't available. Without it the whole feature would be
/// empty on any device without Apple Intelligence.
